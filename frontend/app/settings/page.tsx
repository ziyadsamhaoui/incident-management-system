...
export default function SettingsPage() {
  ...
  const handleDepartmentChange = async (value: string) => {
    ...
    const deptName = dept?.name ?? value;
    setDepartment(value, deptName);
  };
    ...
    <div className="mx-auto w-full max-w-lg px-4 py-6 sm:px-0 sm:py-8 space-y-5">
      ...
      <Card>
        ...
        <CardContent className="space-y-4 px-4 pb-5 sm:px-6">
          {/*  Row 1: Prénom · Nom  - ADMIN can edit, others view-only */}
          ...
          {/*  Row 2: Matricule (locked badge) · Département  */}
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              <label className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider">
                Matricule
              </label>
              <div className="inline-flex items-center gap-1.5 rounded-md border border-slate-300 bg-slate-100 px-2.5 py-1.5 font-mono text-sm font-semibold text-slate-700 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-300">
